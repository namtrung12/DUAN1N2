    ...
    <div class="flex">
        ...
        <main class="flex-1 lg:ml-64 p-4 sm:p-6 lg:p-8">
            ...
            <div class="bg-white rounded-2xl shadow-sm">
                ...
                $endItem = min($page * $perPage, $totalCategories);
                ?>
                <div class="p-6 border-t border-gray-200 flex items-center justify-between">
                    <p class="text-sm text-slate-600">Hiển thị <?= $startItem ?>-<?= $endItem ?> trên <?= $totalCategories ?> kết quả</p>
                    <div class="flex items-center gap-2">
                        <?php if ($page > 1): ?>
                        <a href="<?= BASE_URL ?>?action=admin-categories&page=<?= $page - 1 ?>" class="p-2 hover:bg-gray-100 rounded-lg transition-colors">
                        ...
                        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                            ...
                            <a href="<?= BASE_URL ?>?action=admin-categories&page=<?= $i ?>" class="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-gray-100 text-slate-600 font-semibold transition-colors"><?= $i ?></a>
