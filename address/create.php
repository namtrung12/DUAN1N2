        ...
        <div class="layout-container flex h-full grow flex-col">
            <main class="flex flex-1 justify-center py-10 px-4">
                <div class="w-full max-w-2xl">
                    <?php 
                    $returnUrl = $_SESSION['address_return_url'] ?? 'address';
                    $backUrl = ($returnUrl === 'checkout') ? BASE_URL . '?action=checkout' : BASE_URL . '?action=address';
                    ?>
                    <a href="<?= $backUrl ?>" class="inline-flex items-center gap-1 text-slate-600 font-medium hover:text-primary mb-4 transition-colors">
                    ...
                    <form action="<?= BASE_URL ?>?action=address-store" method="POST" class="bg-white rounded-lg p-6 shadow-sm space-y-4">
                        <?php if (isset($_SESSION['address_return_url'])): ?>
                            <input type="hidden" name="return_url" value="<?= htmlspecialchars($_SESSION['address_return_url'], ENT_QUOTES, 'UTF-8') ?>" />
                        <?php endif; ?>
                        ...
                        <div class="flex flex-col">
                            ...
                            <input class="form-input w-full rounded-lg border border-gray-300 h-14 p-4 focus:outline-0 focus:ring-2 focus:ring-primary/50" id="receiver_name" name="receiver_name" type="text" value="<?= htmlspecialchars($_SESSION['old']['receiver_name'] ?? $_SESSION['user']['name'] ?? '', ENT_QUOTES, 'UTF-8') ?>" required />
                        ...
                        <div class="flex flex-col">
                            ...
                            <input class="form-input w-full rounded-lg border border-gray-300 h-14 p-4 focus:outline-0 focus:ring-2 focus:ring-primary/50" id="phone" name="phone" type="text" value="<?= htmlspecialchars($_SESSION['old']['phone'] ?? $_SESSION['user']['phone'] ?? '', ENT_QUOTES, 'UTF-8') ?>" required />
