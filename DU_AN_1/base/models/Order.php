...
class Order extends BaseModel
{
    ...
    public function cancelWithReason($orderId, $reason)
    {
        $sql = "UPDATE {$this->table} SET status = 'cancelled', cancel_reason = :reason, updated_at = NOW() WHERE id = :id";
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([':id' => $orderId, ':reason' => htmlspecialchars($reason, ENT_QUOTES, 'UTF-8')]);
    }
     ...
     * Gọi method này khi load trang orders hoặc qua cron job
     */
    public function autoCompleteDeliveringOrders()
    {
        // Tìm các đơn hàng đang giao đã quá 30 phút kể từ khi cập nhật
    ...
    /**
     * Lấy các đơn hàng đang giao cần tự động hoàn thành
     */
    public function getDeliveringOrdersToComplete()
    {
