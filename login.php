                    ...
                    <div class="flex flex-col items-center justify-center p-6 sm:p-12 order-2 lg:order-1">
                        <div class="w-full max-w-md">
                            ...
                            <form action="<?= BASE_URL ?>?action=post-login" method="POST" class="space-y-4">
                                <?php 
                                // Lấy email và password từ đăng ký (nếu có)
                                $prefillEmail = $_SESSION['registered_email'] ?? $_SESSION['old']['email'] ?? '';
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <input class="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg text-text-primary dark:text-white focus:outline-0 focus:ring-2 focus:ring-primary/50 border border-gray-300 dark:border-gray-600 bg-background-light dark:bg-gray-700 h-14 placeholder:text-text-secondary p-4 text-base font-normal leading-normal" id="email" name="email" placeholder="Nhập email của bạn" type="email" value="<?= htmlspecialchars($prefillEmail, ENT_QUOTES, 'UTF-8') ?>" required />
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <input class="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg text-text-primary dark:text-white focus:outline-0 focus:ring-2 focus:ring-primary/50 border border-gray-300 dark:border-gray-600 bg-background-light dark:bg-gray-700 h-14 placeholder:text-text-secondary p-4 text-base font-normal leading-normal" id="password" name="password" placeholder="Nhập mật khẩu của bạn" type="password" value="<?= htmlspecialchars($prefillPassword, ENT_QUOTES, 'UTF-8') ?>" required />
                            ...
                            <?php unset($_SESSION['errors'], $_SESSION['old'], $_SESSION['registered_email'], $_SESSION['registered_password']); ?>
                        </div>
                    </div>
