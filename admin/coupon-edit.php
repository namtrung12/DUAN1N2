...
<head>
    ...
    <title>Sửa Mã giảm giá - Chill Drink Admin</title>
        ...
        <main class="flex-1 lg:ml-64 p-4 sm:p-6 lg:p-8">
            <div class="mb-8">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <h1 class="text-3xl font-bold text-slate-900">Sửa Mã giảm giá</h1>
                </div>
            </div>
            ...
            <form action="<?= BASE_URL ?>?action=admin-coupon-update" method="POST" class="bg-white rounded-2xl shadow-sm p-8">
                <input type="hidden" name="id" value="<?= $coupon['id'] ?>"/>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        <label class="block text-sm font-semibold text-slate-900 mb-2">Mã giảm giá *</label>
                        <input type="text" name="code" required value="<?= htmlspecialchars($coupon['code'], ENT_QUOTES, 'UTF-8') ?>"
                    ...
                    <div>
                        ...
                        <select name="type" id="couponType" onchange="updateValueConstraints()" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="fixed" <?= $coupon['type'] === 'fixed' ? 'selected' : '' ?>>Giảm cố định (VNĐ)</option>
                    ...
                    <div>
                        ...
                        <input type="number" name="value" id="couponValue" required min="1" value="<?= $coupon['value'] ?>"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                               placeholder="VD: 50000 hoặc 10"/>
                        <p class="text-xs text-gray-500 mt-1" id="valueHint">Nhập số tiền giảm (VNĐ)</p>
                    </div>

                    <div id="maxDiscountWrapper" class="<?= $coupon['type'] === 'percent' ? '' : 'hidden' ?>">
                        <label class="block text-sm font-semibold text-slate-900 mb-2">Giảm tối đa (VNĐ)</label>
                        <input type="number" name="max_discount" id="maxDiscountInput" min="0" value="<?= $coupon['max_discount'] ?? '' ?>"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    ...
                    <div>
                        ...
                        <input type="number" name="min_order" min="0" value="<?= $coupon['min_order'] ?>"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-900 mb-2">Giới hạn sử dụng</label>
                        <input type="number" name="usage_limit" min="0" value="<?= $coupon['usage_limit'] ?>"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    ...
                    <div>
                        ...
                        <select name="required_rank" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="" <?= empty($coupon['required_rank']) ? 'selected' : '' ?>>Tất cả</option>
                            <option value="bronze" <?= $coupon['required_rank'] === 'bronze' ? 'selected' : '' ?>>Bronze</option>
                            <option value="silver" <?= $coupon['required_rank'] === 'silver' ? 'selected' : '' ?>>Silver</option>
                            <option value="gold" <?= $coupon['required_rank'] === 'gold' ? 'selected' : '' ?>>Gold</option>
                            <option value="diamond" <?= $coupon['required_rank'] === 'diamond' ? 'selected' : '' ?>>Kim cương</option>
                        </select>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-900 mb-2">Điểm đổi</label>
                        <input type="number" name="point_cost" min="0" value="<?= $coupon['point_cost'] ?>"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    ...
                    <div class="md:col-span-2">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="is_redeemable" <?= $coupon['is_redeemable'] ? 'checked' : '' ?> class="w-4 h-4 rounded border-gray-300 text-blue-600"/>
                            <span class="text-sm font-medium text-slate-900">Có thể đổi bằng điểm</span>
                        </label>
                    </div>

                    <?php 
                    $currentStartsAt = $coupon['starts_at'] ? date('Y-m-d\TH:i', strtotime($coupon['starts_at'])) : '';
                    $currentExpiresAt = $coupon['expires_at'] ? date('Y-m-d\TH:i', strtotime($coupon['expires_at'])) : '';
                    $now = date('Y-m-d\TH:i');
                    // Nếu ngày bắt đầu đã qua, giữ nguyên giá trị cũ, nếu không thì min là hiện tại
                    $minStartsAt = ($currentStartsAt && $currentStartsAt < $now) ? $currentStartsAt : $now;
                    ?>
                    <div>
                        ...
                        <input type="datetime-local" name="starts_at" id="startsAt" 
                               ...
                               min="<?= $minStartsAt ?>"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                        <p class="text-xs text-gray-500 mt-1">Chỉ được chọn từ thời điểm hiện tại trở đi</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="expires_at" id="expiresAt"
                               ...
                               min="<?= $currentStartsAt ?: $now ?>"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                        <p class="text-xs text-gray-500 mt-1">Phải sau ngày bắt đầu</p>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="3" 
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                  placeholder="Mô tả chi tiết về mã giảm giá..."><?= htmlspecialchars($coupon['description'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                    </div>

                    <div>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="status" <?= $coupon['status'] ? 'checked' : '' ?> class="w-4 h-4 rounded border-gray-300 text-blue-600"/>
                    ...
                    <div class="md:col-span-2 p-4 bg-gray-50 rounded-lg">
                        <p class="text-sm text-gray-600">
                            <strong>Đã sử dụng:</strong> <?= $coupon['used_count'] ?> / <?= $coupon['usage_limit'] ?: '∞' ?> lần
                        </p>
                    </div>
                </div>
    ...
    <script>
        function updateValueConstraints() {
            ...
            if (type === 'percent') {
                ...
                // Ẩn ô giảm tối đa và reset giá trị
        ...
        // Cập nhật min của ngày hết hạn khi chọn ngày bắt đầu
        document.getElementById('startsAt').addEventListener('change', function() {
            const expiresAt = document.getElementById('expiresAt');
            if (this.value) {
                expiresAt.min = this.value;
                // Nếu ngày hết hạn đang nhỏ hơn ngày bắt đầu, reset nó
                if (expiresAt.value && expiresAt.value < this.value) {
                    expiresAt.value = '';
                }
            }
        });
