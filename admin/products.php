    ...
    <div class="flex">
        ...
        <main class="flex-1 lg:ml-64 p-4 sm:p-6 lg:p-8">
            ...
            <div class="bg-white rounded-2xl shadow-sm">
                ...
                $endItem = min($page * $perPage, $totalProducts);
                ?>
                <div class="p-6 border-t border-gray-200 flex items-center justify-between">
                    <p class="text-sm text-slate-600">Hiển thị <?= $startItem ?>-<?= $endItem ?> trên <?= $totalProducts ?> kết quả</p>
                    <div class="flex items-center gap-2">
                        <?php if ($page > 1): ?>
                        <a href="<?= BASE_URL ?>?action=admin-products&page=<?= $page - 1 ?>" class="p-2 hover:bg-gray-100 rounded-lg transition-colors">
                        ...
                        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                            <?php if ($i == $page): ?>
                            <span class="w-10 h-10 flex items-center justify-center rounded-lg bg-cyan-400 text-white font-semibold"><?= $i ?></span>
                            <?php elseif ($i == 1 || $i == $totalPages || abs($i - $page) <= 2): ?>
                            <a href="<?= BASE_URL ?>?action=admin-products&page=<?= $i ?>" class="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-gray-100 text-slate-600 font-semibold transition-colors"><?= $i ?></a>
