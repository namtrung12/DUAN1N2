...
class AdminController
{
        ...
        try {
            ...
            $statusNotifications = [
                'processing' => [
                    ...
                    'message' => 'Đơn hàng #' . $orderCode . ' đang được xử lý. Chúng tôi sẽ sớm chuẩn bị đơn hàng cho bạn!'
                ],
                'preparing' => [
                    'type' => 'order_preparing',
                    'title' => 'Đơn hàng đang được thực hiện',
                    'message' => 'Đơn hàng #' . $orderCode . ' đang được pha chế. Vui lòng chờ trong giây lát!'
                ],
                'shipped' => [
                    'type' => 'order_shipped',
                    'title' => 'Đơn hàng đã giao cho đơn vị vận chuyển',
                    'message' => 'Đơn hàng #' . $orderCode . ' đã được giao cho đơn vị vận chuyển. Đơn hàng sẽ sớm đến tay bạn!'
                ],
                'delivering' => [
                    'type' => 'order_delivering',
                    'title' => 'Đơn hàng đang được giao',
                    'message' => 'Đơn hàng #' . $orderCode . ' đang được giao đến bạn. Vui lòng chú ý điện thoại!'
                ]
            ];
            
            if (isset($statusNotifications[$newStatus])) {
                $notification = $statusNotifications[$newStatus];
    ...
    public function lockMultipleUsers()
    {
        ...
        // Lấy ID admin hiện tại để loại trừ (không thể tự khóa bản thân)
        $currentUserId = $_SESSION['user']['id'];

        try {
            $count = $this->userModel->lockMultiple($userIds, $currentUserId);
            if ($count > 0) {
                $_SESSION['success'] = "Đã khóa $count tài khoản thành công";
            } else {
                $_SESSION['errors'] = ['lock' => 'Không thể khóa tài khoản của chính mình'];
            }
        } catch (Exception $e) {
    ...
    public function unlockMultipleUsers()
    {
        ...
        try {
            $count = $this->userModel->unlockMultiple($userIds);
            $_SESSION['success'] = "Đã mở khóa $count tài khoản thành công";
        } catch (Exception $e) {
            $_SESSION['errors'] = ['unlock' => 'Mở khóa tài khoản thất bại: ' . $e->getMessage()];
        }
